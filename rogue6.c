...
void carveCorridor(int x1, int y1, int x2, int y2, int isHoriz)
{
    ...
    if (stillHaveX || stillHaveY) {
        ...
        if (newDx != 0 || newDy != 0) {
          if (dx>0 && newDy<0) setCell(x1,y1,"╝"); // right→up
          else if (dx>0 && newDy>0) setCell(x1,y1,"╗"); // upper-right
          else if (dx<0 && newDy<0) setCell(x1,y1,"3");
          else if (dx<0 && newDy>0) setCell(x1,y1,"╔"); // upper-left
          else if (dy>0 && newDx>0) setCell(x1,y1,"╚"); // bottom-left
          else if (dy>0 && newDx<0) setCell(x1,y1,"╝"); // bottom-right
          else if (dy<0 && newDx>0) setCell(x1,y1,"╔"); // upper-left
          else if (dy<0 && newDx<0) setCell(x1,y1,"8"); 
          else setCell(x1,y1,"?");
        }
    ...
    // for that last step, or if it is a corner scenario.
        ...
        if(isHoriz) {
          if(endDy == 0) {
            ...
            else if(endDx < 0) setCell(x1,y1,"═");
            else setCell(x1,y1,"C");
          } else {
            if(endDy > 0) setCell(x1,y1,"╚");
            else if(endDy < 0) setCell(x1,y1,"╔");
            else setCell(x1,y1,"F");
          }
        } else {
          if(endDy == 0) {
            if(endDx > 0) setCell(x1,y1,"╗");
            else if(endDx < 0) setCell(x1,y1,"╔");
            else setCell(x1,y1,"I");
          } else {
            if(endDy > 0) setCell(x1,y1,"║");
            else if(endDy < 0) setCell(x1,y1,"K");
            else setCell(x1,y1,"L");
          }
        }
}
...
void printBigMap() {
    for (int y = 0; y < BIG_SIZE; y++) {
        for (int x = 0; x < BIG_SIZE; x++) {
            ...
            if(strcmp(bigMap[y][x], "═") == 0)
              printf("══");
            else if(strcmp(bigMap[y][x], "╚") == 0)
              printf("╚═");
            else if(strcmp(bigMap[y][x], "╔") == 0 && strcmp(bigMap[y][x+1], " ") != 0) // door
              printf("╔═");
            else if (strcmp(bigMap[y][x], "╬") == 0 && (strcmp(bigMap[y][x+1], "═") == 0 ||
                                                        strcmp(bigMap[y][x+1], "╗") == 0 ||
                                                        strcmp(bigMap[y][x+1], "╝") == 0)) //// door
              printf("╬═");
            else if (strcmp(bigMap[y][x], "─") == 0)
              printf("──");
            else if (strcmp(bigMap[y][x], "┌") == 0 && (strcmp(bigMap[y][x+1], "─") == 0 ||
